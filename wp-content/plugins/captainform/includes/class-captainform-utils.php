...
class Captainform_Utils
{
     ...
     * @param    $action
     */
    public static function curl_call($action){
        /**
        ...
        $args = array();
        $args['action'] = $action;
        $_curl_url = 'http://app.captainform.com/modules/captainform/plugin_track.php?';

        $args['website'] = get_site_url();
        $args['is_multisite'] = is_multisite();
        $args['unique_id'] = self::get_referer_params('unique_id');

        $args['installation_id'] = Captainform_Encrypt::encrypt(get_site_option('captainform_installation_id'));
        $args['installation_key'] = Captainform_Encrypt::encrypt(get_site_option('captainform_installation_id'));
        $args['source'] = self::get_referer_params('source');
